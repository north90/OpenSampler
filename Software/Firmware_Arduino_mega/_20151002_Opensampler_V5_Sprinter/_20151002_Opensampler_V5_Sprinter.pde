...
void loop()
{
  ...
  if(buflen){
   ...
   // added code for OpenSampler
   if (millis()-last_update_fan_pin > timeout_time_for_fan_pin*1000){
...
// BACKLIGHT_____________________________________________________________________________________      
      #if LED_BACKLIGHT_PIN  > -1
      case 206: //M206 LED BACKLIGHT ON
        if (code_seen('S')){
            WRITE(LED_BACKLIGHT_PIN, HIGH);
            analogWrite(LED_BACKLIGHT_PIN, constrain(code_value(),0,255) );
        }
        else {
            WRITE(LED_BACKLIGHT_PIN, HIGH);
        ...
        break;
      case 207: //M207 LED_BACKLIGHT Off
          analogWrite(LED_BACKLIGHT_PIN, 0);
          WRITE(LED_BACKLIGHT_PIN, LOW);
        break;
      #endif
      
            #if LED_BACKLIGHT2_PIN  > -1
      case 306: //M306 LED BACKLIGHT ON
        if (code_seen('S')){
            WRITE(LED_BACKLIGHT2_PIN, HIGH);
            analogWrite(LED_BACKLIGHT2_PIN, constrain(code_value(),0,255) );
        }
        else {
            WRITE(LED_BACKLIGHT2_PIN, HIGH);
            analogWrite(LED_BACKLIGHT2_PIN, 255 );
        }
        break;
      case 307: //M207 LED_BACKLIGHT2 Off
          analogWrite(LED_BACKLIGHT2_PIN, 0);
          WRITE(LED_BACKLIGHT2_PIN, LOW);
        break;
      #endif
// BACKLIGHT_____________________________________________________________________________________   
